...
class DetNet(object):
    ...
    def FisherMatr(self, evParams, return_all=False, **kwargs):
        ...
        for d in self.signals.keys():
            ...
            if self.signals[d].detector.shape == "T" and return_all:
                for i in range(3):
                    allF[d + "_%s" % i] = F_[i]
            elif self.signals[d].detector.shape == "L" and return_all:
                allF[d] = F_[0]
            else:
                allF[d] = F_
        if self.verbose:
    ...
    def optimal_location(self, tcoal, is_tGPS=False):
        ...
        def pattern_fixedtpsi(pars, tc=tc):
            ...
            for d in self.signals.keys():
                ...
                if self.signals[d].detector.shape == "T":
                    for i in range(3):
                        Fp, Fc = self.signals[d].detector.compute_antenna_pattern(
                            theta, phi, t=tc, psi=0, rot=i * 60.0
                        )
                        tmpsum = tmpsum + (Fp ** 2 + Fc ** 2)
                else:
                    Fp, Fc = self.signals[d].detector.compute_antenna_pattern(
                        theta, phi, t=tc, psi=0
                    )
                    tmpsum = tmpsum + (Fp ** 2 + Fc ** 2)
        ...
        if len(self.signals.keys()) == 1:
            ...
            return minimize(
                pattern_fixedtpsi,
                [1.0, 1.0],
                bounds=((0.0, onp.pi), (0.0, 2.0 * onp.pi)),
            ).x
        else:
            ...
            return basinhopping(
                ...
                [1.0, 1.0],
                niter=50,
                minimizer_kwargs={"bounds": ((0.0, onp.pi), (0.0, 2.0 * onp.pi))},
            ).x
    ...
    def WFOverlap(self, WF1, WF2, evParams1, evParams2, res=1000, **kwargs):
        ...
        for d in self.signals.keys():
            overlap_int, SNR1, SNR2 = self.signals[d].WFOverlap(
                WF1, WF2, evParams1, evParams2, res=res, return_separate=True
            )
